    ...
    return (
      <View className={`mb-2 flex-row items-center gap-3 rounded-xl border px-4 py-3 ${isComplete ? 'border-gray-100 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50' : 'border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800'}`}>
        ...
        <View className="flex-1">
          ...
          {task.due_date && (
            <Text className="mt-0.5 text-xs text-gray-400">Due {format(new Date(task.due_date + 'T12:00:00'), 'MMM d')}</Text>
          )}
        </View>
        ...
        <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
          {tasks?.length === 0 && (
            <View className="items-center py-16">
              ...
              <Text className="mt-1 text-sm text-gray-400">Add tasks to stay on top of your trip planning.</Text>
            </View>
          )}
      ...
      <Modal visible={showCreate} animationType="slide" transparent>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} className="flex-1">
          <View className="flex-1 justify-end">
            <View className="rounded-t-3xl bg-white px-5 pb-10 pt-5 dark:bg-gray-900" style={{ maxHeight: '80%' }}>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <Text className="mb-1 text-sm font-semibold text-gray-700 dark:text-gray-300">Due date (optional)</Text>
                <TouchableOpacity
