...
-- =====================
-- Phase 11: Polls, Tasks, Itinerary
-- =====================

-- Helper: check if the current user can access a trip (group member or solo owner)
...
create table if not exists public.trip_tasks (
  ...
  created_by uuid references public.users not null,
  title text not null,
  category text not null default 'other' check (category in ('flights', 'hotel', 'activities', 'transport', 'packing', 'other')),
